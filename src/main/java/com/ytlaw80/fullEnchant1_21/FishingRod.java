...
public class FishingRod implements CommandExecutor {

    @Override
    public boolean onCommand(CommandSender sender, @NotNull Command command, String s, String[] args) {
        sender.sendMessage(ChatColor.AQUA+"낚싯대의 최대 마법부여 + 마법부여대 사용 가능 여부:");
        sender.sendMessage(ChatColor.GOLD+"바다의 행운 Luck of the sea   3(III) "+ ChatColor.GREEN+"  (O)");
        sender.sendMessage(ChatColor.GOLD+"미끼 Lure   3(III) "+ ChatColor.GREEN+"  (O)");
