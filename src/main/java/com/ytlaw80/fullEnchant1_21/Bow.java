...
public class Bow implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, @NotNull Command command, String s, String[] args) {
        sender.sendMessage(ChatColor.AQUA+"활의 최대 마법부여 + 마법부여대 사용 가능 여부:");
        sender.sendMessage(ChatColor.GOLD+"힘 Power   5(V) "+ ChatColor.GREEN+"  (O)");
        sender.sendMessage(ChatColor.GOLD+"화염 Flame   1(I) "+ ChatColor.GREEN+"  (O)");
        sender.sendMessage(ChatColor.GOLD+"무한 Infinity   1(I) "+ ChatColor.GREEN+"  (O)");
        ...
        sender.sendMessage(ChatColor.GOLD+"밀어내기 Punch   3(III) "+ ChatColor.GREEN+"  (O)");
