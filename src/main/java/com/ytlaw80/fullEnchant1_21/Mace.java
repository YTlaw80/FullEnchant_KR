...
public class Mace implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, @NotNull Command command, String s, String[] args) {
        sender.sendMessage(ChatColor.AQUA+"철퇴의 최대 마법부여 + 마법부여대 사용 가능 여부:");
        sender.sendMessage(ChatColor.GOLD+"육중 Density   5(V) "+ ChatColor.GREEN+"  (O)");
        sender.sendMessage(ChatColor.GOLD+"격파 Breach   4(IV) "+ ChatColor.GREEN+"  (O)");
        ...
        sender.sendMessage(ChatColor.GOLD+"돌풍 Wind Burst   3(III) "+ ChatColor.RED+"  (X)");
