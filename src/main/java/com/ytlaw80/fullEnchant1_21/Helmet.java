...
public class Helmet implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, @NotNull Command command, String s, String[] args) {
        sender.sendMessage(ChatColor.AQUA+"헬멧의 최대 마법부여 + 마법부여대 사용 가능 여부:");
        sender.sendMessage(ChatColor.GOLD+"보호 Protection   4(IV) "+ ChatColor.GREEN+"  (O)");
        sender.sendMessage(ChatColor.GOLD+"호흡 Respiration   3(III) "+ ChatColor.GREEN+"  (O)");
        sender.sendMessage(ChatColor.GOLD+"친수성 Aqua Affinity   1(I) "+ ChatColor.GREEN+"  (O)");
